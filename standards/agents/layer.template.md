...
- **Канон-шаблон в prompt** — конкретный образец под слой ([[../canon/architecture/layers]] сигнатуры). Даёшь копипаст-образец, не отсылаешь к докам.
...
- **Compliance в prompt**: каждый агент знает свои «нельзя» ([[../canon/architecture/import-rules]]).
- **Один уточняющий вопрос максимум.**
- **`export default <Name>`** обязателен ([[../canon/architecture/layers]] конвенция export).
