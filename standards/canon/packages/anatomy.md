...
- **Интерактив через тег-флоу** ([ui-proxy-tag-flow](../architecture/ui-proxy-tag-flow.md)): пакетные View/Widget рисуются проксированным `Ui`, события всплывают в app-Feature по тегу. Пакет теги не навязывает — апп может перемапить (aliases). Именованная доменная семантика — через `useEmit` (opt-in).
...
- **Импорты** — стандартные HCA-правила ([import-rules](../architecture/import-rules.md)): no upward/horizontal; направление к `core/` как к точке координации.
...
- [ ] Проходит эталон-гейт ([etalon-gate](../principles/etalon-gate.md)): тесты + трейсы + доки + раскладка.
