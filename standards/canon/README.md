...
- `containers-only-execution.md` — исполнение ТОЛЬКО в контейнере («нет докера — нет работы»); файлы на машине (bind-mount workspace), движуха в докере; Docker Engine, не Desktop; версии — пинам репо, не образу.
