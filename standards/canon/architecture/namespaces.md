...
Живут в сгенерированном `OmnifieldSlots` (`.omnifield/@types/slots.d.ts`). Каждое property типизировано как `typeof import('@<layer>/...').default` — Ctrl+Click ведёт в источник. Требует `export default` в каждом файле слоя (см. [layers](layers.md) конвенция export).
